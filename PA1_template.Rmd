...
```
The mean of total (`r meanOfTotal`) is much smaller than median of total (`r medianOfTotal`)
...
From above picture, we can see interval `r intervalWithMaxMean` contains the maximum number of steps, on average across all the days in the dataset.
...
There is `r totalNARowNum` rows with NA steps in the dataset.
...
Compare with `r meanOfTotal` and `r medianOfTotal`, after imputing missing value the mean of number of steps taken the day is still `r meanOfTotal_noNA`, and median is still `r medianOfTotal_noNA`.
